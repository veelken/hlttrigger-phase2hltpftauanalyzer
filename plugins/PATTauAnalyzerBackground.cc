#include "HLTrigger/TallinnHLTPFTauAnalyzer/plugins/PATTauAnalyzerBackground.h"
...
PATTauAnalyzerBackground::PATTauAnalyzerBackground(const edm::ParameterSet& cfg)
...
PATTauAnalyzerBackground::~PATTauAnalyzerBackground()
{}

std::vector<BaseTau> 
PATTauAnalyzerBackground::buildBaseTaus(const edm::Event& evt, const edm::EventSetup& es)
{
  ...
  { 
    ...
    if ( (min_pt_ < 0. || pfTau.pt()                              >= min_pt_ ) &&
         ...
         (                pfTau.leadChargedHadrCand().isNonnull()            ) )
    {
...
DEFINE_FWK_MODULE(PATTauAnalyzerBackground);
