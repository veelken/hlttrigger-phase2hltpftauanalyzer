...
TH1* getHistogram(DQMStore& dqmStore, const std::string& meName, bool& dqmError)
{
  ...
  if ( me ) {
    MonitorElement::Kind meType = me->kind();
    if ( meType == MonitorElement::Kind::TH1F      ||
         meType == MonitorElement::Kind::TH1S      ||
         meType == MonitorElement::Kind::TH2F      ||
         meType == MonitorElement::Kind::TH2S      ||
         meType == MonitorElement::Kind::TH3F      ||
         meType == MonitorElement::Kind::TPROFILE  ||
         meType == MonitorElement::Kind::TPROFILE2D ) {
      try {
        histogram = me->getTH1();
      } catch ( cms::Exception& ex ) {
        edm::LogError ("getHistogram")
            << " Exception triggered when trying to access histogram associated to meName = " << meName << ","
            << " MonitorElement type = " << (int)meType << ":" << ex.what() << " !!";
        return histogram;
      }
      if ( !histogram ) {
        edm::LogError ("getHistogram")
...
double getValue(DQMStore& dqmStore, const std::string& meName_full, bool& error)
{
  ...
  if ( me ) {
    MonitorElement::Kind meType = me->kind();
    if      ( meType == MonitorElement::Kind::REAL ) return me->getFloatValue();
    else if ( meType == MonitorElement::Kind::INT  ) return me->getIntValue();
    else {
                           ...
                           double scaleFactor, double scaleFactorErr, int mode)
{
  ...
  if ( meType == MonitorElement::Kind::TH1F       ||
    ...
    if ( !(meType == MonitorElement::Kind::TPROFILE   ||
           meType == MonitorElement::Kind::TPROFILE2D) ) {
      if ( scaleFactorErr > 0. ) {
  ...
  } else if ( meType == MonitorElement::Kind::INT ) {
  ...
  } else if ( meType == MonitorElement::Kind::REAL ) {
  ...
  } else if ( meType == MonitorElement::Kind::STRING ) {
                        ...
                        std::vector<outputCommandEntry>* outputCommands)
{
  ...
  for ( std::vector<std::string>::const_iterator dirName = dirNames.begin();
    ...
    dqmCopyRecursively(dqmStore, inputDirName_full, outputDirName_full,
                       scaleFactor, scaleFactorErr, mode, outputCommands);
  }
}
...
{
  MonitorElement::Kind meType = me->kind();
  if ( meType == MonitorElement::Kind::INT ) {
