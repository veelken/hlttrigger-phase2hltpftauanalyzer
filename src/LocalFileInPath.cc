...
/// class; other code should not even know about them!
  ...
  LocalFileInPath::LocalFileInPath() :
    relativePath_(),
    canonicalFilename_(),
    location_(Unknown)
  {
  ...
  LocalFileInPath::LocalFileInPath(const std::string& r) :
    relativePath_(r),
    canonicalFilename_(),
    location_(Unknown)
  {
  ...
  LocalFileInPath::LocalFileInPath(char const* r) :
    relativePath_(r ? r : ""),
    canonicalFilename_(),
    location_(Unknown)
  {
  ...
  LocalFileInPath::LocalFileInPath(LocalFileInPath const& other) :
  ...
  LocalFileInPath&
  LocalFileInPath::operator=(LocalFileInPath const& other)
  {
    LocalFileInPath temp(other);
    this->swap(temp);
    return *this;
  }

  void
  LocalFileInPath::swap(LocalFileInPath& other)
  {
  ...
  std::string
  LocalFileInPath::relativePath() const
  {
    return relativePath_;
  }


  LocalFileInPath::LocationCode
  LocalFileInPath::location() const
  {
    return location_;
  }

  std::string
  LocalFileInPath::fullPath() const
  {
    return canonicalFilename_;
  }

  void
  LocalFileInPath::write(std::ostream& os) const
  {
  ...
  void
  LocalFileInPath::read(std::istream& is)
  {
    std::string vsn;
    std::string relname;
    ...
    if (oldFormat) {
      ...
      int loc;
      is >> relname >> loc;
      location_ = static_cast<LocalFileInPath::LocationCode>(loc);
  ...
  void
  LocalFileInPath::readFromParameterSetBlob(std::istream& is)
  {
    std::string vsn;
    std::string relname;
    std::string canFilename;
  ...
  void
  LocalFileInPath::getEnvironment() {
  ...
  LocalFileInPath::initialize_() {
    ...
    typedef std::vector<std::string> stringvec_t;
    stringvec_t  pathElements = edm::tokenize(searchPath_, ":");
	...
	for (bf::path br = pathPrefix.branch_path(); !br.normalize().string().empty(); br = br.branch_path()) {
	  ...
	  //---------------------------------------------------------------------
          // CV: needed to make ttHAnalysis workflow run on Tallinn batch system
          location_ = Local;
          return;
          //---------------------------------------------------------------------
	}
    ...
    throw edm::Exception(edm::errors::FileInPathError)
      << "LocalFileInPath unable to find file "
      << relativePath_
