...
#include <string>
  ...
  public:
    ...
    explicit LocalFileInPath(const std::string& r);
    explicit LocalFileInPath(const char* r);

    LocalFileInPath(LocalFileInPath const& other);
    LocalFileInPath& operator=( LocalFileInPath const& other);
    ~LocalFileInPath();
    void swap(LocalFileInPath& other);
  ...
  inline
  void
  swap(LocalFileInPath& a, LocalFileInPath& b) 
  {
    a.swap(b);
  }

  inline  std::ostream& 
  operator<< (std::ostream& os, const LocalFileInPath& fip)
  {
    fip.write(os);
    return os;
  }
  ...
  inline std::istream&
  operator>> (std::istream& is, LocalFileInPath& fip)
  {
    fip.read(is);
    return is;
  }
  ...
  inline bool
  operator== (LocalFileInPath const& a,
	      LocalFileInPath const& b)
  {
